...
let close_typer shared =
  ...
  Shared.blocking_put shared.msg (Msg `Closing);
  if debug_lvl > 1 then
...
let share_exn shared exn =
  ...
  Shared.blocking_put shared.msg (Msg (`Exn exn));
  if debug_lvl > 1 then
...
let cancel_typer shared =
  ...
  Shared.blocking_put shared.msg (Msg `Cancel);
  if debug_lvl > 1 then
...
let process config shared =
  ...
  | effect Motyper.Partial (Run evals), k ->
      ...
      Shared.blocking_put shared.msg (Partial evals);
    ...
    | Config config ->
        (if debug_lvl > 0 then
         ...
         try
           ...
           | All -> Shared.blocking_put shared.msg (Partial pipeline)
           | _ -> (* Already shared *) ()
         with
         | Closing_exn ->
             if debug_lvl > 0 then
         ...
         | Cancel_exn ->
             ...
             ()
         | Exception_after_partial ->
             if debug_lvl > 0 then
               Format.printf "%sCaught an exception after partial result.\n%!"
                 (Utils.domain_name ());
             ()
...
let get shared config =
  Shared.blocking_put shared.msg (Config config);
